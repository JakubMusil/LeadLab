...
function activityTypeLabel(type: string): string {
  const map: Record<string, string> = {
    comment: t('recordDetail.typeComment'),
    call: t('recordDetail.typeCall'),
    meeting: t('recordDetail.typeMeeting'),
    email_out: t('recordDetail.typeEmailOut'),
    email_in: t('recordDetail.typeEmailIn'),
    task: t('recordDetail.typeTask'),
    task_assigned: t('recordDetail.typeTaskAssigned'),
    task_completed: t('recordDetail.typeTaskCompleted'),
    task_reopened: t('recordDetail.typeTaskReopened'),
    status_change: t('recordDetail.typeStatusChange'),
    file_upload: t('recordDetail.typeFileUpload'),
    proposal_created: t('recordDetail.typeProposalCreated'),
    proposal_accepted: t('recordDetail.typeProposalAccepted'),
    proposal_rejected: t('recordDetail.typeProposalRejected'),
    entity_change: t('recordDetail.typeEntityChange'),
    sms_out: t('recordDetail.typeSmsOut'),
    sms_in: t('recordDetail.typeSmsIn'),
    whatsapp_out: t('recordDetail.typeWhatsAppOut'),
    whatsapp_in: t('recordDetail.typeWhatsAppIn'),
    meeting_scheduled: t('recordDetail.typeMeetingScheduled'),
    call_scheduled: t('recordDetail.typeCallScheduled'),
    event_scheduled: t('recordDetail.typeEventScheduled'),
    task_expired: t('recordDetail.typeTaskExpired'),
    link: t('recordDetail.typeLink'),
    voice_memo: t('recordDetail.typeVoiceMemo'),
    system_note: t('recordDetail.typeSystemNote'),
    todo_items_added: t('recordDetail.typeTodoItems'),
    checklist: t('recordDetail.typeChecklist'),
  }
...
const actionPickerItems = computed<{ value: string; label: string; icon: Component }[]>(() => {
  ...
  return [
    ...registryItems,
    { value: 'task', label: t('recordDetail.typeTask'), icon: ClipboardDocumentListIcon },
  ]
})
...
async function doDeleteActivity() {
  ...
  if (!res.ok) {
    toast.error(t('recordDetail.activityDeleteFailed'))
    return
  }
  ...
  <div class="space-y-4" data-testid="activity-timeline">
    ...
    <div v-if="!hideComposer" class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-4" data-testid="activity-timeline-composer">
      ...
      <div v-if="!selectedActionType">
        <p class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2.5">{{ t('recordDetail.chooseActionType') }}</p>
      ...
      <div v-else-if="selectedActionType !== 'task'" class="flex flex-col gap-2">
        <div class="flex items-center gap-2 mb-1">
          ...
          >← {{ t('recordDetail.changeType') }}</button>
        </div>
        <RichTextEditor
          ref="richTextEditorRef"
          v-model="newActivityText"
          :placeholder="selectedActionType === 'comment' ? t('recordDetail.commentPlaceholder') : t('recordDetail.notePlaceholder')"
        ...
        <div class="flex items-center justify-between gap-2">
          <label class="flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-300 select-none cursor-pointer">
            ...
            />
            <span>{{ t('recordDetail.markInternal') }}</span>
          </label>
          <button
            ...
            @click="addActivity"
          >{{ activitySubmitting ? '…' : t('recordDetail.activitySubmit') }}</button>
        </div>
      </div>
      ...
      <div v-else class="space-y-3">
        <div class="flex items-center gap-2 mb-1">
          ...
          <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('recordDetail.typeTask') }}</span>
          <button
            class="ml-auto text-xs text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-colors"
            @click="selectedActionType = ''"
          >← {{ t('recordDetail.changeType') }}</button>
        ...
        <div class="flex justify-end">
          <button
            ...
            @click="addTask"
          >{{ taskSubmitting ? '…' : t('recordDetail.addTask') }}</button>
        </div>
      </div>
    </div>
    ...
    >
      {{ activities.length > 0 ? t('recordDetail.noActivitiesForFilter') : t('recordDetail.noActivities') }}
    </div>
        ...
        >
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2 pt-3 flex-wrap">
              <span class="text-xs font-semibold text-gray-700 dark:text-gray-300">{{ t('recordDetail.typeTask') }}</span>
              ...
              <div class="flex flex-wrap items-center gap-1.5 mt-2">
                ...
                <div class="relative">
                  <button
                    ...
                    :title="t('recordDetail.addReaction')"
          ...
          <template v-if="(item as unknown as Activity).is_deleted">
            ...
            <div class="min-w-0 flex-1">
              <p class="text-xs text-gray-400 dark:text-gray-500 italic">
                {{ t('recordDetail.activityDeleted', {
                  user: (item as unknown as Activity).deleted_by_name ?? t('recordDetail.unknownUser'),
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <template v-if="item.type === 'meeting_scheduled' || item.type === 'call_scheduled' || item.type === 'event_scheduled'">
                ...
                >{{ t(`recordDetail.scheduledTaskStatus.${((item.tool_payload as Record<string, unknown>).task_status as string)}`) }}</span>
              </template>
              <span
                ...
                :title="t('recordDetail.activityInternalTooltip')"
              >{{ t('recordDetail.activityInternal') }}</span>
              ...
              <div class="ml-auto opacity-0 group-hover/row:opacity-100 transition-opacity flex items-center gap-2">
                ...
                <button
                  ...
                  :title="t('recordDetail.editActivity', 'Upravit')"
                ...
                <button
                  ...
                  :title="t('recordDetail.deleteActivity')"
              ...
              <ul class="space-y-1">
                ...
                >
                  <button
                    ...
                    :title="chItem.done ? t('recordDetail.checklistUncheck', 'Uncheck') : t('recordDetail.checklistCheck', 'Check')"
    ...
    >{{ t('recordDetail.loadMore') }}</button>
  </div>
