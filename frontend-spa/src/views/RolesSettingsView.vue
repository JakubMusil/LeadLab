...
// Role wizard (replaces old inline create form + presets modal)
...
function onWizardCreated() {
  // Roles list is already updated by the store; nothing else to do here.
}
  ...
  <div class="space-y-8">
    ...
    <div>
      <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100 mb-3">
        {{ t('permissions.systemAccountType') }}
      </h3>
      <div class="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead class="bg-gray-50 dark:bg-gray-800">
            <tr>
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ t('permissions.accountTypeIdentifier') }}</th>
    ...
    <div>
      <div class="flex items-center justify-between mb-3">
        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">
          {{ t('permissions.customAccountType') }}
        </h3>
        <div class="flex gap-2">
          ...
          <button
            v-if="permissionsStore.canManageRoles && customRoles.length > 0"
          ...
          >
            ...
            {{ t('permissions.createAccountType') }}
          </button>
        </div>
      </div>

      <!-- Welcome / empty state for custom roles -->
      <div
        v-if="customRoles.length === 0 && !loading"
        class="rounded-2xl border border-dashed border-gray-300 dark:border-gray-600 p-8 text-center bg-gradient-to-br from-amber-50/50 to-white dark:from-amber-900/10 dark:to-gray-900"
      >
        <SparklesIcon class="h-8 w-8 mx-auto text-amber-500" aria-hidden="true" />
        <h4 class="mt-3 text-sm font-semibold text-gray-900 dark:text-gray-100">
          {{ t('permissions.welcomeAccountTypesTitle') }}
        </h4>
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400 max-w-md mx-auto">
          {{ t('permissions.welcomeAccountTypesBody') }}
        </p>
        <button
        ...
        >
          <PlusIcon class="h-4 w-4" />
          {{ t('permissions.welcomeAccountTypesCta') }}
        </button>
      </div>
    ...
    <ConfirmDeleteModal
      ...
      :title="t('permissions.deleteAccountType')"
      :message="t('permissions.deleteAccountTypeConfirm')"
      @confirm="deleteRole"
      @cancel="pendingDeleteRole = null"
    />

    <!-- New: Role wizard (replaces presets + create form) -->
    <RoleWizard
      :open="showWizard"
