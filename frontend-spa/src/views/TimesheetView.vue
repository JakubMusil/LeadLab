  ...
  <div class="p-6">
    ...
    <div v-if="showManualForm" class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 mb-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div class="relative">
          <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Record (optional)</label>
          <input v-model="recordSearch" type="text" placeholder="Search records…"
            @input="searchRecords(recordSearch)"
            class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
          <button v-if="form.record_id" class="absolute right-2 top-7 text-gray-400 hover:text-gray-600 text-xs" @click="clearRecord"><XMarkIcon class="w-4 h-4" /></button>
          <div v-if="recordResults.length" class="absolute z-10 w-full mt-0.5 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg max-h-40 overflow-y-auto">
            <button v-for="r in recordResults" :key="r.id"
