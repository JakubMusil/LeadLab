...
# ---------------------------------------------------------------------------
# Task Expired — auto-generated by Celery when a task passes its grace period
# ---------------------------------------------------------------------------

class TaskExpiredTool(_SimpleLogTool):
    """
    Logged automatically by ``auto_expire_scheduled_tasks`` when a calendar-
    bound Task transitions to the terminal ``expired`` status.

    Not user-composable — this tool has no UI button in the sidebar action
    picker.  It is only created programmatically so the entity timeline stays
    auditable after an automatic state transition.
    """

    activity_type = "task_expired"
    label = _("Task Expired")
    icon = "ClockIcon"
    category = "task"
    default_visibility = "secondary"
    schema_properties = {
        "task_id": {"type": "string", "title": "Task ID"},
        "task_title": {"type": "string", "title": "Task Title"},
        "task_kind": {"type": "string", "title": "Task Kind"},
        "due_date": {"type": "string", "title": "Due Date"},
        "expired_at": {"type": "string", "title": "Expired At"},
    }
    payload_fields = ["task_id", "task_title", "task_kind", "due_date", "expired_at"]
...
BUILTIN_TOOLS: list[StreamlineTool] = [
    ...
    ChecklistTool(),
    # Task Expired — auto-generated by Celery (not user-composable)
    TaskExpiredTool(),
]
