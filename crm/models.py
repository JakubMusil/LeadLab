...
class Customer(SoftDeleteMixin, TenantModel):
    ...
    created_by = models.ForeignKey(
        ...
        help_text="The user who created this contact.",
    )
    assigned_to = models.ForeignKey(
        ...
        help_text="The account owner / person responsible for this contact.",
    )
...
class PipelineRecord(SoftDeleteMixin, TenantModel):
    ...
    TOOLBAR_TOOLS: list[str] = [
        # Communication — user-composable actions
        "comment",
        "call",
        "meeting",
        "email_out",
        ...
        "whatsapp_in",
        "chat",
        # Planning — user-composable actions
        "meeting_scheduled",
        "call_scheduled",
        "event_scheduled",
        "task",
        ...
        "time_logged",
        "approval_requested",
        # Files & references — user-composable actions
        "file_upload",
        "voice_memo",
        "link",
        # Commerce — user-composable actions
        "payment_received",
        "invoice_sent",
        "signature_requested",
        # Removed (auto-logged by system/integrations, not user-composable):
        # "mention"          — users mention via @ in comments
        # "approval_resolved" — resolved programmatically, not from toolbar
        # "signature_completed" — auto-logged by e-sign integrations
        # "system_note"      — generated by system only
    ]
...
class Activity(models.Model):
    ...
    * CALL / MEETING       → {"duration_minutes": 0, "notes": "..."}
    ...
    # metadata are cleared, but a tombstone entry remains in the timeline.
...
# ---------------------------------------------------------------------------
# Phase 2 — Unified task timeline (replaced by Activity with task FK)
# ---------------------------------------------------------------------------
# TimelineEventType, TaskTimelineEntry, TaskCommentReaction, and
# TaskVoiceAttachment have been removed. Use Activity(task=...) with the
# corresponding ActivityType value instead.
...
class Proposal(SoftDeleteMixin, TenantModel):
    ...
    created_by = models.ForeignKey(
        ...
        help_text="The user who created this proposal.",
    )
    assigned_to = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        null=True,
        ...
        help_text="The person responsible for closing / following up on this proposal.",
    )
